...
namespace CPRDGOLD.mappers
{
    internal class Provider : Mapper<Provider>
    {
        ...
        protected override void LoadData(dynamic refSource = null)
        {
            ...
            string[] cols =
            {
                "provider_id", "specialty_concept_id", "care_site_id", "gender_concept_id", "provider_source_value",
                "specialty_source_value", "gender_source_value",
            };
            var items = StaffLoader.GetData().Select(staff =>
            {
                var sconcept = SourceToConceptMapLoader.ByJobCode(staff.role == 0 ? null : staff.role.ToString()) ??
                               new SourceToConceptMap();
                var lu = LookupLoader.ByCodeType(staff.role == 0 ? null : staff.role.ToString(), 76) ?? new Lookup();
                return new Provider
                {
