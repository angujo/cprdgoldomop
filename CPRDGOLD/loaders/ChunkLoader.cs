...
namespace CPRDGOLD.loaders
{
    public abstract class ChunkLoader<T, C> : Loader<T, C> where T : new()
    {
        ...
        public ChunkLoader(string table_name, Chunk chunk) : base(DB.Source, table_name)
        {
            this.chunk = chunk;
            chunk.AddCleaner(() => Clean());
        }
        ...
        public void CustomizeQuery(Query query, string schema_name)
        {
            switch (typeof(T).Name)
            {
                case nameof(ConsultationLoader):
                    query.SelectRaw(
                        "row_number() over(partition by consultation.patid, consultation.eventdate order by consultation.eventdate) AS consult_row");
                ...
                case nameof(PatientLoader):
                    query.Join($"{schema_name}.practice",
                               j => j.WhereRaw("RIGHT(patient.patid::varchar,5)::numeric = practice.pracid"))
                         .SelectRaw(
                             "greatest(patient.frd,practice.uts) AS op_start_date, least(patient.tod,practice.lcd, patient.frd) AS op_end_date, 32880 AS pt_concept_id");
                    break;
                    ...
                    query.Join($"{schema_name}.product", "product.prodcode", "therapy.prodcode")
                         .LeftJoin($"{schema_name}.source_to_standard",
                                   j => j.On("source_to_standard.source_code", "product.gemscriptcode")
                                         .WhereNull("source_to_standard.target_invalid_reason")
                                         .WhereRaw(
                                             "therapy.eventdate between source_to_standard.source_valid_start_date and source_to_standard.source_valid_end_date")
                                         .Where("source_to_standard.target_standard_concept", "S")
                         ...
                         .LeftJoin($"{schema_name}.source_to_source",
                                   j => j.On("source_to_source.source_code", "product.gemscriptcode")
                                         .WhereRaw(
                                             "therapy.eventdate between source_to_source.source_valid_start_date and source_to_source.source_valid_end_date")
        ...
        public void LoopAllFast(Action<C> looper)
        {
            ...
            chunk.Log.Info($"Total Data Chunk to LoopAll [{dataset.Count}] [{typeof(T).Name}]");
            Parallel.ForEach(
                dataset, new ParallelOptions {MaxDegreeOfParallelism = 10, CancellationToken = Runner.Token},
                c => looper(c));
            chunk.Log.Info($"Finished Chunk LoopAll #ChunkLoader [{typeof(T).Name}]");
        }
        ...
        public static new T LoopAll(Action<C> looper)
        {
        ...
        protected C DataTableValue(object keys) => IDataTableValue(keys);
