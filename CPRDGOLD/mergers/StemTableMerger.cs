            ...
            chunk.GetLoader<TherapyLoader>(ChunkLoadType.THERAPY).LoopAllFast(ther =>
               {
                   ...
                   DaySupplyDecode ddecode = DaySupplyDecodeLoader.ByAll((int)ther.prodcode, Int32.TryParse($"{cdosage?.daily_dose}", out int dd) ? dd : 0, (int?)ther.qty, (int?)ther.numpacks) ?? new DaySupplyDecode();
